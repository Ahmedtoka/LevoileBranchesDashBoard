...
<div class="modal fade" id="userModal" tabindex="-1">
    <div class="modal-dialog">
        <form class="modal-content" method="POST" id="userForm">
            ...
            <div class="modal-header"><h6 class="modal-title" id="uTitle">{{ t('users.new','مستخدم جديد') }}</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="mb-2"><label class="form-label small">{{ t('common.name','الاسم') }}</label><input name="name" id="uName" class="form-control" required></div>
                <div class="mb-2"><label class="form-label small">{{ t('common.email','البريد') }}</label><input name="email" id="uEmail" type="email" class="form-control" required></div>
                <div class="row g-2">
                    <div class="col-6"><label class="form-label small">{{ t('common.phone','الهاتف') }}</label><input name="phone" id="uPhone" class="form-control"></div>
                    <div class="col-6"><label class="form-label small">{{ t('common.password','كلمة المرور') }} <span class="text-muted" id="uPwHint"></span></label><input name="password" id="uPassword" type="text" class="form-control"></div>
                </div>
                <div class="row g-2 mt-1">
                    <div class="col-6">
                        <label class="form-label small">{{ t('common.role','الدور') }}</label>
                    ...
                    <div class="col-6">
                        <label class="form-label small">{{ t('common.department','الإدارة') }}</label>
                ...
                <div class="mt-2">
                    <label class="form-label small">{{ t('users.branch_single','الفرع') }} <span class="text-muted">({{ t('users.branch_single_hint','فرع مدير الفرع الواحد') }})</span></label>
                ...
                <div class="mt-2">
                    <label class="form-label small">{{ t('users.covered','الفروع المغطّاة') }} <span class="text-muted">({{ t('users.covered_hint','منطقة الأريا مانجر / فروع الفني — متعدّد') }})</span></label>
                    <select name="branch_ids[]" id="uBranches" class="form-select" multiple size="6">
                        @foreach($branches as $b)<option value="{{ $b->id }}">{{ $b->branch_name }}</option>@endforeach
                    </select>
                    <div class="form-text">{{ t('users.tech_auto','للفني: الطلبات في الفروع دي بتتسند له تلقائيًا. Ctrl/Cmd-click لاختيار أكتر من فرع.') }}</div>
                </div>
                <div class="form-check mt-2"><input type="checkbox" name="is_department_manager" value="1" class="form-check-input" id="uMgr"><label class="form-check-label small" for="uMgr">{{ t('users.is_manager','مدير إدارة') }}</label></div>
                <div class="form-check"><input type="checkbox" name="active" value="1" class="form-check-input" id="uActive" checked><label class="form-check-label small" for="uActive">{{ t('common.active','نشط') }}</label></div>
            </div>
            <div class="modal-footer"><button class="btn btn-primary">{{ t('users.save','حفظ المستخدم') }}</button></div>
        </form>
    </div>
</div>
@endsection
...
function prepUser() {
    ...
    document.getElementById('uTitle').textContent = @json(t('users.new','مستخدم جديد'));
    ...
    document.getElementById('uPwHint').textContent = @json(t('users.pw_required','(مطلوبة)'));
}
document.querySelectorAll('.edit-user').forEach(btn => btn.addEventListener('click', () => {
    ...
    document.getElementById('uTitle').textContent = @json(t('users.edit','تعديل مستخدم'));
    document.getElementById('uName').value = u.name || '';
    document.getElementById('uEmail').value = u.email || '';
    document.getElementById('uPhone').value = u.phone || '';
    document.getElementById('uPassword').value = '';
    document.getElementById('uPwHint').textContent = @json(t('users.pw_keep','(اتركها فارغة للإبقاء)'));
